...
1. Explain that this reset removes tracked maintainer history from `PROGRESS.md`, `.agent/decisions.md`, `wiki/log.md`, and `.agent/plans/` / `.agent/progresses/` / `.agent/decisions/` artifacts.
2. Run `bash scripts/reset-template-state.sh` from the repo root.
3. Run `bash scripts/validate-wiki.sh` after the reset.
4. Report whether the repo is structurally valid and whether maintainer-specific plan, progress, per-plan decision, legacy decision, and log history is gone.
