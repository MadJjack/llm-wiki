...
4. Ensure promised wiki updates are complete, Task Router routing is still correct for any new or changed pages, `wiki/log.md` has the relevant entries, `PROGRESS.md` moves the plan from Active to Completed, and `.agent/context.md` is refreshed.
5. Archive the plan/progress/decision trio only after all validations pass and the completion checklist is fully true.
